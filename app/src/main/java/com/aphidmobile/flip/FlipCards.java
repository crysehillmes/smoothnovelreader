...
public class FlipCards {
    ...
    private boolean isFullOverFlipCalled = false;
                ...
                if (state == STATE_TOUCH) {
                    ...
                    if (frontCards.getIndex() == maxIndex - 1) { //the last page
                        if (accumulatedAngle > frontCards.getIndex() * 180) {
                            ...
                            if(controller.isOverFlipEnabled() && !isFullOverFlipCalled && this.state != STATE_AUTO_ROTATE && controller.getOnOverFlipListener() != null) {
                                if(Float.compare(accumulatedAngle, frontCards.getIndex() * 180 + MAX_TIP_ANGLE) == 0) {
                                    isFullOverFlipCalled = true;
                                    controller.getOnOverFlipListener().onOverFlip(true, true, accumulatedAngle);
                                } else {
                                    controller.getOnOverFlipListener().onOverFlip(true, false, accumulatedAngle);
                                }
                            }
                        }
                    }
                    ...
                    if (accumulatedAngle < 0) {
                        ...
                        if(controller.isOverFlipEnabled() && !isFullOverFlipCalled && this.state != STATE_AUTO_ROTATE && controller.getOnOverFlipListener() != null) {
                            if(Float.compare(accumulatedAngle, -MAX_TIP_ANGLE) == 0) {
                                isFullOverFlipCalled = true;
                                controller.getOnOverFlipListener().onOverFlip(false, true, accumulatedAngle);
                            } else {

                                controller.getOnOverFlipListener().onOverFlip(false, false, accumulatedAngle);
                            }
                        }
                    }
